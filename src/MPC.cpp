...
// we need a reference velocity to  dealing with stopping
double ref_v = 70;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  vars[x_start] = x; // x_start = 0
  vars[y_start] = y; // y_start = x_start + N
  vars[psi_start] = psi; // psi_start = y_start + N
  vars[v_start] = v;  // v_start = psi_start + N
  vars[cte_start] = cte; // cte_start = v_start + N
  vars[epsi_start] = epsi;  // epsi_start = cte_start + N
